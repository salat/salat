...
import com.github.salat.json.{FromJValue, ToJField}
import com.github.salat.util.{Logging, _}
import com.github.salat.{Field => SField}
...
abstract class Grater[X <: AnyRef](val clazz: Class[X])(implicit val ctx: Context) extends Logging {
  ...
  def fromBSON(bytes: Array[Byte]): X = {
    ...
    val mdbo = com.github.salat.bson.BSONObjectToMongoDbObject(decoded)
    asObject(mdbo)
  }
