...
import com.github.salat.json.{StrictJSONDateStrategy, StringDateStrategy, TimestampDateStrategy, _}
  ...
  "JSON support" should {
    "handle converting model objects to JObject" in {
      ...
      "serialize class hierarchies" in {
        ...
        "with a top-level trait" in {
          ...
          i_* must /("_t" -> "com.github.salat.test.json.Ivar")
          i_* must /("s" -> "Hello")
          val j = Johan(s = "Hello", d = 3.14)
          val j_* = grater[Helge].toPrettyJSON(j)
          j_* must /("_t" -> "com.github.salat.test.json.Johan")
          j_* must /("s" -> "Hello")
          j_* must /("d" -> 3.14)
        }
        "with an abstract superclass" in {
          ...
          l_* must /("_t" -> "com.github.salat.test.json.Ludvig")
          l_* must /("s" -> "Hello")
          val m = Martin(s = "Hello", d = 3.14)
          val m_* = grater[Kalle].toPrettyJSON(m)
          m_* must /("_t" -> "com.github.salat.test.json.Martin")
          m_* must /("s" -> "Hello")
          m_* must /("d" -> 3.14)
        }
      }
    ...
    "handle converting a list of model objects to a JArray" in {
      ...
      jarr.arr must contain(exactly(
        JObject(JField("_t", JString("com.github.salat.test.json.Martin")) :: JField("s", JString("one")) :: JField("d", JDouble(1.1)) :: Nil).asInstanceOf[JValue],
        JObject(JField("_t", JString("com.github.salat.test.json.Martin")) :: JField("s", JString("two")) :: JField("d", JDouble(2.2)) :: Nil).asInstanceOf[JValue],
        JObject(JField("_t", JString("com.github.salat.test.json.Martin")) :: JField("s", JString("three")) :: JField("d", JDouble(3.3)) :: Nil).asInstanceOf[JValue]
      )).inOrder
    }
    ...
    "deserialize JObjects containing class hierarchies" in {
      "with a top-level trait" in {
        grater[Helge].fromJSON(JObject(
          JField("_t", JString("com.github.salat.test.json.Ivar")) ::
            JField("s", JString("Hello")) ::
            Nil
        )) must_== Ivar(s = "Hello")
        grater[Helge].fromJSON(JObject(
          JField("_t", JString("com.github.salat.test.json.Johan")) ::
            JField("s", JString("Hello")) ::
      ...
      "with an abstract superclass" in {
        grater[Kalle].fromJSON(JObject(
          JField("_t", JString("com.github.salat.test.json.Ludvig")) ::
            JField("s", JString("Hello")) ::
            Nil
        )) must_== Ludvig(s = "Hello")
        grater[Kalle].fromJSON(JObject(
          JField("_t", JString("com.github.salat.test.json.Martin")) ::
            JField("s", JString("Hello")) ::
            JField("d", JDouble(3.14)) ::
            Nil
    ...
    "strings" in {
      ...
      "a string that can be parsed to a JSON array" in {
        val arr = """[{"_t":"com.github.salat.test.json.Martin","s":"one","d":1.1},{"_t":"com.github.salat.test.json.Martin","s":"two","d":2.2},{"_t":"com.github.salat.test.json.Martin","s":"three","d":3.3}]"""
    ...
    "JArray" in {
      val j = JArray(
        JObject(JField("_t", JString("com.github.salat.test.json.Martin")) :: JField("s", JString("one")) :: JField("d", JDouble(1.1)) :: Nil) ::
          JObject(JField("_t", JString("com.github.salat.test.json.Martin")) :: JField("s", JString("two")) :: JField("d", JDouble(2.2)) :: Nil) ::
          JObject(JField("_t", JString("com.github.salat.test.json.Martin")) :: JField("s", JString("three")) :: JField("d", JDouble(3.3)) :: Nil) ::
          Nil
      )
