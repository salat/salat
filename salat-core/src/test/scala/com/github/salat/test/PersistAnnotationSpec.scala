...
class PersistAnnotationSpec extends SalatSpec {
  ...
  "com.github.salat.annotations.Persist" should {
    ...
    "allow a field inside a case class to be persisted" in {
      ...
      "a value that requires a transformer" in {
        ...
        //        log.info(MapPrettyPrinter(dbo))
        dbo must havePair("_typeHint" -> "com.github.salat.test.model.Maud2")
        dbo must havePair("swept" -> "swept")
        dbo must havePair("out" -> "out")
        dbo must havePair("ida" -> {
          val builder = MongoDBObject.newBuilder
          builder += "_typeHint" -> "com.github.salat.test.model.Ida"
          builder += "lake" -> 8.0
          builder.result
        })
      ...
      "a var" in {
        ...
        //        log.info(MapPrettyPrinter(dbo))
        dbo must havePair("_typeHint" -> "com.github.salat.test.model.Maud3")
        dbo must havePair("swept" -> "swept")
        dbo must havePair("out" -> "out")
    ...
    "respect @Persist declared in a trait" in {
      ...
      dbo must havePair("_typeHint" -> "com.github.salat.test.model.Maud8")
      dbo must havePair("swept" -> "swept")
      dbo must havePair("out" -> "out")
    ...
    "respect @Persist declared in immediate superclass" in {
      ...
      dbo must havePair("_typeHint" -> "com.github.salat.test.model.Maud11")
      dbo must havePair("swept" -> "swept")
      dbo must havePair("out" -> "out")
    ...
    "work with @Salat on a trait" in {
      ...
      "where @Persist is declared in the subclasses implementing the trait" in {
        ...
        //      log.info(MapPrettyPrinter(dbo))
        dbo must havePair("_typeHint" -> "com.github.salat.test.model.ManyMauds")
        dbo must havePair("mauds" -> {
          val listBuilder = MongoDBList.newBuilder
          listBuilder += {
            val builder = MongoDBObject.newBuilder
            builder += "_typeHint" -> "com.github.salat.test.model.Maud5"
            builder += "swept" -> "swept"
            builder += "out" -> "out"
          ...
          listBuilder += {
            val builder = MongoDBObject.newBuilder
            builder += "_typeHint" -> "com.github.salat.test.model.Maud6"
            builder += "swept" -> "swept"
            builder += "out" -> "out"
            builder += "ida" -> {
              val builder = MongoDBObject.newBuilder
          ...
          listBuilder += {
            val builder = MongoDBObject.newBuilder
            builder += "_typeHint" -> "com.github.salat.test.model.Maud7"
            builder += "swept" -> "swept"
            builder += "out" -> "out"
      ...
      "where a collection is typed to a trait declaring @Persist" in {
        ...
        //        log.info(MapPrettyPrinter(dbo))
        dbo must havePair("_typeHint" -> "com.github.salat.test.model.Maudelic")
        dbo must havePair("mauds" -> {
          val listBuilder = MongoDBList.newBuilder
          listBuilder += {
            val builder = MongoDBObject.newBuilder
            builder += "_typeHint" -> "com.github.salat.test.model.Maud8"
            builder += "swept" -> "swept"
            builder += "out" -> "out"
            builder += "toSea" -> "tuo tpews"
          ...
          listBuilder += {
            val builder = MongoDBObject.newBuilder
            builder += "_typeHint" -> "com.github.salat.test.model.Maud9"
            builder += "swept" -> "swept"
            builder += "out" -> "out"
            builder += "ida" -> {
              val builder = MongoDBObject.newBuilder
          ...
          listBuilder += {
            val builder = MongoDBObject.newBuilder
            builder += "_typeHint" -> "com.github.salat.test.model.Maud10"
            builder += "swept" -> "swept"
            builder += "out" -> "out"
            // notWaving does not have @Persist so does not appear here
            builder += "butDrowning" -> true
